...
const DOCS_RECORD_VIDEO = (() => {
  ...
  return String(process.env.RECORD_VIDEO ?? "true").trim().toLowerCase() !== "false";
})();
...
function resolveCaptureTimeoutMs() {
  ...
  const minFromBudget = (budgetS + 90) * 1000;
  const explicit = process.env.DOCS_CAPTURE_TIMEOUT_MS;
  if (explicit != null && String(explicit).trim() !== "") {
    return Math.max(Number(explicit) || 150000, minFromBudget);
  }
  if (isRenderDeploy()) return Math.max(DOCS_RECORD_VIDEO ? 300000 : 120000, minFromBudget);
  if (isDocsCaptureHealEnabled() && isCaptureMcpHealEnabled()) return Math.max(180000, minFromBudget);
  if (isDocsCaptureHealEnabled()) return Math.max(150000, minFromBudget);
  return Math.max(isDocsGenFast() ? 120000 : 150000, minFromBudget);
}

function resolveCaptureBudgetS() {
  const explicit = process.env.DOCS_CAPTURE_BUDGET_S;
  if (explicit != null && String(explicit).trim() !== "") {
    return Number(explicit) || 120;
  }
  if (isRenderDeploy()) return DOCS_RECORD_VIDEO ? 240 : 90;
  if (isDocsCaptureHealEnabled()) return 180;
  const captureFast = String(process.env.DOCS_CAPTURE_FAST ?? "true").trim().toLowerCase() !== "false";
  return isDocsGenFast() ? 120 : captureFast ? 100 : 120;
}

function attemptTimeoutMs(baseMs, attempt) {
  const scaled = baseMs * (1 + 0.25 * (attempt - 1));
  return Math.min(Math.round(scaled), 300000);
}
...
function docsCaptureEnv(sessionId) {
  ...
  const defaultPostNavWait = "2.5";
  const defaultVideoWalkthrough = DOCS_RECORD_VIDEO ? (onRender ? "8" : "12") : onRender ? "0" : "12";
  const defaultAddOrderVideo = DOCS_RECORD_VIDEO ? (onRender ? "14" : "18") : onRender ? "0" : "18";
  return {
    ...
    E2E_FAST: onRender ? "0" : "1",
    LOGIN_WAIT_S: onRender ? "60" : String(process.env.LOGIN_WAIT_S || "30"),
...
async function captureScreenshots(
  ...
  moduleName,
  description = "",
  timeoutMs = DOCS_CAPTURE_TIMEOUT_MS,
  extraEnv = {}
) {
...
async function captureObserveOnly(sessionId, moduleName, description = "", extraEnv = {}) {
  return captureScreenshots(sessionId, moduleName, description, DOCS_CAPTURE_TIMEOUT_MS, {
    ...extraEnv,
