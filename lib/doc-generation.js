...
const DOCS_RECORD_VIDEO =
  String(process.env.DOCS_RECORD_VIDEO ?? "true").trim().toLowerCase() !== "false";
...
function resolveCaptureHealAttempts() {
  ...
  return isDocsCaptureHealEnabled() ? 5 : 1;
}

function resolveCaptureTimeoutMs() {
  const explicit = process.env.DOCS_CAPTURE_TIMEOUT_MS;
  if (explicit != null && String(explicit).trim() !== "") {
    return Number(explicit) || 150000;
  }
  if (isDocsCaptureHealEnabled() && isCaptureMcpHealEnabled()) return 180000;
  if (isDocsCaptureHealEnabled()) return 150000;
  return isDocsGenFast() ? 120000 : 150000;
}
  ...
  return {
    ...
    DOCS_CAPTURE_ALLOW_ORDER_ADD_URL:
      process.env.DOCS_CAPTURE_ALLOW_ORDER_ADD_URL != null
        ...
        : "true",
    DOCS_CAPTURE_POST_NAV_WAIT_S: String(process.env.DOCS_CAPTURE_POST_NAV_WAIT_S || "2.5"),
    DOCS_VIDEO_WALKTHROUGH_S: String(process.env.DOCS_VIDEO_WALKTHROUGH_S || "12"),
    DOCS_ADD_ORDER_VIDEO_S: String(process.env.DOCS_ADD_ORDER_VIDEO_S || "18"),
...
async function captureScreenshotsWithHeal(
  ...
  const mcpHeal = isCaptureMcpHealEnabled();
  const effectiveMax = healEnabled
    ? Math.max(maxAttempts, Number(process.env.DOCS_CAPTURE_HEAL_ATTEMPTS || 5))
    : maxAttempts;
