...
function resolveCaptureHealAttempts() {
  ...
  if (isRenderDeploy()) return isDocsCaptureHealEnabled() ? 2 : 1;
...
function resolveCaptureTimeoutMs() {
  ...
  if (isRenderDeploy()) return 90000;
...
function docsCaptureEnv(sessionId) {
  ...
  const defaultPostNavWait = onRender ? "1.5" : "2.5";
  const defaultVideoWalkthrough = onRender ? "0" : "12";
  const defaultAddOrderVideo = onRender ? "0" : "18";
  return {
...
async function captureScreenshotsWithHeal(
  ...
  const effectiveMax = healEnabled
    ? Math.max(
        maxAttempts,
        Number(
          process.env.DOCS_CAPTURE_HEAL_ATTEMPTS ||
            (isRenderDeploy() ? 2 : 5)
        )
      )
    : maxAttempts;
