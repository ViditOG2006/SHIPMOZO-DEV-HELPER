...
function resolveCaptureBudgetS() {
  ...
  if (explicit != null && String(explicit).trim() !== "") {
    ...
    budget = DOCS_RECORD_VIDEO ? 240 : 90;
  ...
  if (DOCS_RECORD_VIDEO && budget < 240) budget = 240;
  return budget;
}
...
function resolveCaptureTimeoutMs() {
  ...
  const videoFloor = DOCS_RECORD_VIDEO ? 360000 : 0;
  const explicit = process.env.DOCS_CAPTURE_TIMEOUT_MS;
  if (explicit != null && String(explicit).trim() !== "") {
    return Math.max(Number(explicit) || 150000, minFromBudget, videoFloor);
  }
  if (isRenderDeploy()) {
    return Math.max(DOCS_RECORD_VIDEO ? 360000 : 120000, minFromBudget, videoFloor);
  }
