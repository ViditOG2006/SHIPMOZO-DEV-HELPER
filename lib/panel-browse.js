...
function runPanelBrowse(query, sessionId = chatSessionId()) {
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      ...
      finish({
        ok: false,
        error: `Panel browse timed out after ${Math.round(BROWSE_TIMEOUT_MS / 1000)}s`,
        sessionId,
        query,
        pages: [],
        screenshots: [],
...
async function browsePanelForChat(query) {
  const sessionId = chatSessionId();
  const result = await runPanelBrowse(query, sessionId);
...
function appendScreenshotsIfMissing(reply, screenshots) {
  if (!screenshots?.length) return reply;
...
function mergeScreenshots(liveShots, manualShots) {
  const urls = new Set();
  const out = [];
  for (const s of [...(liveShots || []), ...(manualShots || [])]) {
