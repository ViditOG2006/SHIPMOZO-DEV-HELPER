...
async function runNavigationScenario(scenario) {
  ...
  for (const list of lists) {
    for (const s of list || []) {
      if (s?.url && !urls.has(s.url)) {
        urls.add(s.url);
        out.push({ id: s.id, label: s.label, url: s.url, storage: s.storage });
      }
    }
  }
  return out;
...
async function attachEvidence(scenario, ctx, result) {
  if (!ctx.captureEvidence || !ctx.runId) return result;
  if (result.screenshots?.length >= 1) return result;
  try {
    const evidence = await captureScenarioEvidence(ctx.runId, scenario);
...
async function executeScenario(scenario, ctx) {
  ...
  try {
    let skipReason = null;
    if (ctx.skipLive && scenario.category === "screenshots") {
    ...
    switch (scenario.category) {
      ...
      case "module_docs":
        ...
        break;
      case "navigation":
        actual = await runNavigationScenario(scenario);
        break;
      case "e2e":
        actual = await runApiScenario(scenario, ctx);
    ...
    const done = baseResult(scenario, {
      ...
      actual: {
        ...
        screenshotCount: actual.screenshotCount,
...
async function runTestStep({
  ...
  const ctx = { skipLive, model, provider, runId, captureEvidence };
...
async function runTestDataset({
  ...
  const ctx = { skipLive, model, provider, runId, captureEvidence: doCaptureEvidence };
  const results = [];
  ...
  const run = {
    ...
    options: { skipLive, captureEvidence: doCaptureEvidence, scenarioCount: scenarios.length },
    summary,
    results,
  };
