...
function isDocsCaptureHealEnabled() {
  const raw = String(process.env.DOCS_CAPTURE_HEAL_ENABLED ?? "true").trim().toLowerCase();
  if (raw === "false" || raw === "0" || raw === "off") return false;
  if (raw === "true" || raw === "1" || raw === "yes") return true;
  if (process.env.DOCS_CAPTURE_HEAL_PROVIDER) return true;
  const attempts = Number(process.env.DOCS_CAPTURE_HEAL_ATTEMPTS || 0);
  if (attempts > 1) return true;
  // Deprecated Claude flag — treat explicit false as off, true as on
